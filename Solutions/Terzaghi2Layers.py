...
sys.path.append("./PhysicalPropertyTools")
...
class Solution(object):
    def __init__(self, height_1, height_2, tao_0, rock, fluid, p_0_1=0, p_0_2=0):
        self.tao_0 = tao_0;
        ...
        self.c_s_1 = rock.fromMaterialGetProperty("PAY", "Compressibility");
        self.permeability_1 = rock.fromMaterialGetProperty("PAY", "Permeability");
        self.phi_1 = rock.fromMaterialGetProperty("PAY", "Porosity");
        self.nu_1 = rock.fromMaterialGetProperty("PAY", "PoissonsRatio")
        self.nu_u_1 = rock.fromMaterialGetProperty("PAY", "PoissonsRatio")
        self.G_1 = rock.fromMaterialGetProperty("PAY", "ShearModulus");
        ...
        self.nu_u_1 = self._calculate_ni_u(self.nu_1, self.alpha_1, self.B_1)
        self.K_ni_u_1 = self._calculate_K_ni_u(self.K_u_1, self.nu_u_1)
        ...
        self.c_s_2 = rock.fromMaterialGetProperty("NONPAY", "Compressibility");
        self.permeability_2 = rock.fromMaterialGetProperty("NONPAY", "Permeability");
        self.phi_2 = rock.fromMaterialGetProperty("NONPAY", "Porosity");
        self.ni_2 = rock.fromMaterialGetProperty("NONPAY", "PoissonsRatio")
        self.ni_u_2 = rock.fromMaterialGetProperty("NONPAY", "PoissonsRatio")
        self.G_2 = rock.fromMaterialGetProperty("NONPAY", "ShearModulus");

        self.K_s_2 = self._calculate_K_s(self.c_s_2)
        self.K_phi_2 = self._calculate_K_phi(self.K_s_2)
        self.K_2 = self._calculate_K(self.G_2, self.ni_2)
        ...
        self.c_2 = self._calculate_c(self.permeability_2, self.G_2, self.ni_2, self.ni_u_2, self.mu, self.alpha_2)
    ...
    def _calculate_ni_u(self, ni, alpha, B):
        value = ((3.0 * ni + alpha * B * (1.0 - 2.0 * ni)) /
                  (3.0 - alpha * B * (1.0 - 2.0 * ni)));
        return value

    def _calculate_K_ni_u(self, K_u, ni_u):
        return (3.0 * K_u * (1.0 - ni_u)) / (1.0 + ni_u);

    def _calculate_gama(self, B, ni_u):
        return (B * (1.0 + ni_u)) / (3.0 * (1.0 - ni_u));
    ...
    def _calculate_K_v_u(self, K_u, ni_u):
        return 3*K_u*(1 - ni_u) / (1 + ni_u)

    def _calculate_c_m(self, alpha, ni, K):
        return (alpha * (1.0 + ni)) / (3.0 * K * (1.0 - ni));
